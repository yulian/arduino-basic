/*
 * 초음파(Ultrasonic) 센서를 이용해 거리를 측정하는 예제 코드 (초음파 센서의 리드선이 4개인 경우)
 * 
...
int PIN_ULTRASONIC_TRIG = 2;  // 초음파 센서 TRIG 핀 초기화
int PIN_ULTRASONIC_ECHO = 3;  // 초음파 센서 ECHO 핀 초기화
...
void loop() {
  ...
  pinMode(PIN_ULTRASONIC_TRIG, OUTPUT);   // 트리거 핀을 출력 모드로 선언
  digitalWrite(PIN_ULTRASONIC_TRIG, LOW); // 트리거 핀에 전압을 0V로 하여 동작하지 않게 함
  delayMicroseconds(2);                   // 송출 신호와 수신 신호 사이에 일정시간 지연
  digitalWrite(PIN_ULTRASONIC_TRIG, HIGH);// 트리거 핀에 전압을 가하여 초음파를 송출함
  delayMicroseconds(5);                   // 5 마이크로초(0.005 밀리초) 동안 초음파를 송출함
  digitalWrite(PIN_ULTRASONIC_TRIG, LOW); // 초음파 송출 정지


  pinMode(PIN_ULTRASONIC_ECHO, INPUT);    // 에코 핀을 입력 모드로 전환
